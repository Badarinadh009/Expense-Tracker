    ...
    <div className='grid grid-cols-2'>
        <div className='bg-yellow-400 w-[full] h-[100vh] flex flex-col p-4 items-center text-center'>
            ...
            <div className='h-[200px] flex items-end justify-center gap-6 pb-2'>
                ...
                <input className='border-2' onKeyDown={(e)=>e.key==='Enter'? addTransaction :''} type='number' placeholder='Enter the amount' value={amount} onChange={(e)=>setAmount(e.target.value)} />
            </div>
      ...
      <div className='bg-black text-amber-400 p-4'>
        <div className='flex justify-between font-bold text-xl h-[100px]'>
        ...
        <ul className='grid grid-cols-3 pt-4'>
            {transaction.map((value, index)=>(
                ...
                <span  className='cursor-pointer pl-2'onClick={()=>deleteTransaction(value.id)}>X</span></li>
            ))}
        </ul>
      </div>
    </div>
